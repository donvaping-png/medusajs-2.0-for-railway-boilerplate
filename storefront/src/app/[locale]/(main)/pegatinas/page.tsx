...
export async function generateMetadata({
  ...
  return {
    ...
    description: category?.description || "Pegatinas personalizadas de alta calidad",
  }
}
...
export default async function PegatinasPage({
  ...
  const { locale } = await params
  const category = getCategory("pegatinas")
  const subcategories = getSubcategories("pegatinas")
    ...
    <main className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {subcategories.map((subcategory) => (
          <Link
            key={subcategory.id}
            href={`/${locale}/pegatinas/${subcategory.slug}`}
